    ...
    // Unterscheidung Bezahlt oder Unbezahlt (aber nur 1 res. Platz)
      ...
      WHERE b.party_id = %int% AND s.userid = %int% AND s.status = %string%", $party->party_id, $userid, $seat_status);
        ...
        if (!$row['blockid']) {
            ...
            return "<a href=\"#\" onclick=\"javascript:var w=window.open('index.php?mod=seating&action=popup&design=popup&function=usrmgr&id={$row['blockid']}&userarray[]=$userid&l=1','_blank','resizable=yes');\" class=\"small\">$LinkText</a>";
        }
    }
   ...
   WHERE s.userid=%int% AND s.status = %string% AND b.party_id = %int%", $userid, $seat_status, $party->party_id);
    ...
    public function CoordinateToBlockAndName($x, $y, $blockid, $MaxBlockLength = 0, $LinkIt = 0, $userid = 0)
    {
        ...
        if (!$row['name']) {
            ...
            if ($LinkIt == 2) {
                return "<a href=\"#\" onclick=\"javascript:var w=window.open('index.php?mod=seating&action=popup&design=popup&function=usrmgr&id=$blockid&userarray[]={$userid}&l=1','_blank','resizable=yes');\">$LinkText</a>";
            } else {
                return $LinkText;
            }
        }
    ...
    // Has user paid?
      ...
      ';
        ...
        if ($mode == 0) {
            ...
            if ($selected_user) {
                $jscode .= "CreateText('". t('Auswahl') ."', ". 14 .", ". ($YOffset + 126) .", '');\n";
            } else {
                $jscode .= "CreateText('". t('Dein Platz') ."', ". 14 .", ". ($YOffset + 126) .", '');\n";
            }
